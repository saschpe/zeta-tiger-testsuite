...
wenn die JWT-Assertion fehlerhaft ist, z.B. durch ein falsches Format, fehlende Claims oder fehlende Attestierungsdaten in der ``client_assertion``.
Dies umfasst auch den Fall, dass bei einer neuen Session nach Ablauf des Refresh Tokens die erneute Client-Attestierung im ``client_statement`` der ``client_assertion`` fehlt.
...
----
+
Im Fehlerfall enthält die ``client_assertion`` keine verwertbare Software-Attestierung, weil die Attestierungsdaten im ``client_statement``
fehlen, entfernt oder unvollständig sind.
...
- Die ``client_assertion`` oder das Token ist fehlerhaft.
+
